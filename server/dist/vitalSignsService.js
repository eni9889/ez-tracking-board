...
class VitalSignsService {
     ...
     */
    async getEncounterById(encounterId, patientId, accessToken) {
        try {
            const response = await axios_1.default.get(`${this.EZDERM_BASE_URL}/ezderm-webservice/rest/encounter/getById/_rid/${encounterId}`, {
                headers: {
                    'Host': 'srvprod.ezinfra.net',
                    'accept': 'application/json',
                    'authorization': `Bearer ${accessToken}`,
     ...
     */
    async findMostRecentVitalSigns(encounters, patientId, accessToken) {
    ...
    async processVitalSignsCarryforward(encounter, accessToken) {
        try {
            ...
            const historicalEncounters = await this.getHistoricalEncounters(encounter.patientInfo.id, accessToken, encounter.id);
            if (historicalEncounters.length === 0) {
                console.log(`No historical encounters found for patient ${encounter.patientInfo.id}`);
                await database_1.vitalSignsDb.markAsProcessed(encounter.id, encounter.patientInfo.id, undefined, undefined, undefined, undefined, undefined, false, 'No historical encounters found');
                return false;
            }
            // Find most recent encounter with vital signs
            const recentVitalSigns = await this.findMostRecentVitalSigns(historicalEncounters, encounter.patientInfo.id, accessToken);
            if (!recentVitalSigns) {
                console.log(`No historical vital signs found for patient ${encounter.patientInfo.id}`);
                await database_1.vitalSignsDb.markAsProcessed(encounter.id, encounter.patientInfo.id, undefined, undefined, undefined, undefined, undefined, false, 'No historical vital signs found');
                return false;
            }
            // Get current encounter with vital signs to update
            const currentEncounter = await this.getEncounterById(encounter.id, encounter.patientInfo.id, accessToken);
            if (!currentEncounter) {
                console.log(`Could not get current encounter ${encounter.id}`);
                await database_1.vitalSignsDb.markAsProcessed(encounter.id, encounter.patientInfo.id, recentVitalSigns.encounter.id, undefined, undefined, undefined, undefined, false, 'Could not get current encounter');
                return false;
            }
            ...
            if (!currentVitalSigns) {
                ...
                await database_1.vitalSignsDb.markAsProcessed(encounter.id, encounter.patientInfo.id, recentVitalSigns.encounter.id, undefined, undefined, undefined, undefined, false, 'Current encounter has no vital signs record');
                return false;
            }
            // Update with historical height and weight
            const updatedVitalSigns = { ...currentVitalSigns };
            ...
            const updateSuccess = await this.updateVitalSigns(updatedVitalSigns, accessToken, encounter.patientInfo.id);
            if (updateSuccess) {
                ...
                await database_1.vitalSignsDb.markAsProcessed(encounter.id, encounter.patientInfo.id, recentVitalSigns.encounter.id, recentVitalSigns.vitalSigns.height1, recentVitalSigns.vitalSigns.weight1, recentVitalSigns.vitalSigns.heightUnit, recentVitalSigns.vitalSigns.weightUnit, true);
                return true;
            }
            else {
                await database_1.vitalSignsDb.markAsProcessed(encounter.id, encounter.patientInfo.id, recentVitalSigns.encounter.id, recentVitalSigns.vitalSigns.height1, recentVitalSigns.vitalSigns.weight1, recentVitalSigns.vitalSigns.heightUnit, recentVitalSigns.vitalSigns.weightUnit, false, 'Failed to update vital signs in EZDerm');
                return false;
            }
        }
        catch (error) {
