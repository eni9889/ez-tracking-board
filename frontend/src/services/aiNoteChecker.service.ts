...
class AINoteCheckerService {
  ...
  async getEligibleEncounters(): Promise<EligibleEncounter[]> {
    if (USE_MOCK_DATA) {
      console.log('🚧 Development Mode: Returning mock eligible encounters');
      await new Promise(resolve => setTimeout(resolve, 500)); // Simulate API delay
      return this.mockEligibleEncounters;
    }
            ...
            items: [
              {
                ...
                text: '1. Psoriasis vulgaris\n\t•\tProblem: Chronic - Stable\n\t•\tPlan: Continue current topical therapy',
                autogenerated: true,
                maxCodingValue: 4,
  ...
  async processAllEligibleNotes(): Promise<BatchProcessResult> {
    if (USE_MOCK_DATA) {
      console.log('🚧 Development Mode: Simulating batch processing');
  ...
  async getNoteCheckResults(limit: number = 50, offset: number = 0): Promise<NoteCheckResult[]> {
    if (USE_MOCK_DATA) {
      console.log('🚧 Development Mode: Returning mock note check results');
  ...
  async getNoteCheckResult(encounterId: string): Promise<NoteCheckResult | null> {
    if (USE_MOCK_DATA) {
      console.log('🚧 Development Mode: Finding mock note check result');
